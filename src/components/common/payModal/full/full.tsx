...
export default function FullPayment({ flatData, activeButton, onNext, isSubmitting = false }: FullPaymentProps) {
    ...
    const amountsForRows = ((): number[] => {
        if (n === 0) return [total];
        const firstAmounts = sortedPaymentDates.slice(0, -1).map((d) => parsePriceString(amountByDateKey[calendarDateToRu(d)] ?? "0"));
        const sumFirst = firstAmounts.reduce((a, b) => a + b, 0);
        const lastAmount = Math.max(0, total - sumFirst);
                    ...
                    {bonusPhoneStep === "phone" && (
                        <>
                            ...
                            <Button
                                ...
                                className="w-full rounded-[12px] bg-[#1A3C7E] text-white text-[14px] font-medium"
                            >
                            ...
                            {sortedPaymentDates.map((d, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isLast ? (
                                            <span className="text-[#000] text-[14px] opacity-80">
                                                {t("remaining")}: {formatPriceDisplay(amountsForRows[i] ?? 0)}
                                            </span>
                                        ) : (
