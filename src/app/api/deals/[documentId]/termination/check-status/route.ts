...
export async function POST(
  ...
  try {
    ...
    if (templateTypeFromSignedAgreementRecord(saRow) !== "Расторжение") {
    ...
    // Казреестр для расторжения временно отключён (API у контрагента ещё не готов).
    // try {
    ...
    // } catch (kzErr: any) {
    //   console.warn(
    //     "[deals/termination/check-status] kazreestr register:",
