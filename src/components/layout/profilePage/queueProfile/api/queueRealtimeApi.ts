...
type QueueRealtimeEvent = "queue" | "managerStatus";
...
export async function subscribeToQueueBranchUpdates(
  ...
  try {
    ...
    const listenedEvents = options?.listen ?? ["queue", "managerStatus"];
    const shouldListenQueueEvents = listenedEvents.includes("queue");
    const shouldListenManagerStatusEvents = listenedEvents.includes("managerStatus");
    let lastRefreshAt = 0;
    const safeRefresh = () => {
      const now = Date.now();
    ...
    if (shouldListenManagerStatusEvents) {
