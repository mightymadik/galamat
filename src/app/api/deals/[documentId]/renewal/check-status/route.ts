      ...
      try {
        ...
        if (fileRes.ok) {
          ...
          const pdfFilename = agreementNumberForPdf
            ? `Переоформление ${agreementNumberForFilename(agreementNumberForPdf)}.pdf`
            : `signed-renewal-${doodocsDocumentId}.pdf`;
        ...
        console.warn("[deals/renewal/check-status] upload link_ddcard failed:", uploadErr?.message);
      }
    }
