...
export function TerminationSignStep({
  ...
  const handleSendToSign = async () => {
    ...
    try {
      const genRes = await fetch(`/api/deals/${encodeURIComponent(dealDocumentId)}/termination/generate`, {
        method: "POST",
      ...
      const cust = data?.deal?.customer;
      const clientEmail = (cust?.email ?? "").trim();
      const clientIin = String(cust?.iin ?? "").replace(/\D/g, "").padStart(12, "0").slice(-12);
      if (!clientEmail || clientIin.length !== 12) {
        setSignError("У клиента должны быть заполнены email и ИИН (12 цифр)");
        setSendingToSign(false);
        return;
      }
      const sendRes = await fetch(`/api/deals/${encodeURIComponent(dealDocumentId)}/termination/send-to-sign`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          ...
          clientEmail,
          clientIin,
          contractName: agreementNumber ? `Расторжение ${agreementNumber.replace(/\//g, "-")}` : "Расторжение",
        }),
      });
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      const res = await fetch(`/api/deals/${encodeURIComponent(dealDocumentId)}/termination/check-status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      if (res.ok && json?.signed) {
        ...
        setSignError("Документ ещё не подписан");
      }
    } catch {
      setSignError("Ошибка сети");
    } finally {
  ...
  const handleComplete = async () => {
    ...
    setCompleting(true);
    try {
      const res = await fetch(`/api/deals/${encodeURIComponent(dealDocumentId)}/termination/complete`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({}),
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        setSignError(json?.error ?? "Ошибка завершения расторжения");
        return;
      }
      onComplete();
    } catch {
  ...
  return (
    <div className="flex flex-col items-start gap-6 self-stretch">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[#122C5E] text-base">Подписать договор расторжения</span>
      </div>
      {sentToSign && !signedAt && (
        <p className="text-[#2655AF] text-sm">
          Документ отправлен на подписание. Порядок: 1) клиент, 2) компания. После подписания всеми нажмите «Обновить статус».
        </p>
      )}
      {signedAt && (
      ...
      <div className="flex flex-col gap-3 self-stretch">
        ...
        >
          <span className="text-white text-[15px] font-medium">
            {completing ? "Загрузка…" : !signedAt ? "Сначала подпишите документ" : "Завершить расторжение"}
          </span>
        </Button>
      </div>
    </div>
