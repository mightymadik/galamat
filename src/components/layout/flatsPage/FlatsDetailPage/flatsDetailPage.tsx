    ...
    useEffect(() => {
        ...
        const savedId =
            typeof sessionStorage !== "undefined"
                ? (() => {
                    try {
                        const raw = sessionStorage.getItem("payDealContext");
                        if (raw) {
                            const parsed = JSON.parse(raw);
                            if (parsed?.dealDocumentId) return String(parsed.dealDocumentId);
                        }
                    } catch { }
                    return sessionStorage.getItem("payDealId");
                })()
                : null;
        if (!savedId) {
            setResumeDealId(null);
                ...
                <div className="wrapper flex items-center gap-[32px] flex-shrink-0 flex-wrap">
                            ...
                            {activePlan === "Планировка" ? (
                                flat.images && flat.images.length > 0 ? (
                                    ...
                                    <div className="w-[598px] h-[534px] bg-gray-200 rounded-[12px] flex items-center justify-center">
                                        ...
                                        {realEstateType === "parking" && (
                                            <svg xmlns="http://www.w3.org/2000/svg" width="164" height="164" viewBox="0 0 28 31" fill="none">
                    ...
                    <div className="flex w-full lg:max-w-[463px] p-[32px] flex-col items-start flex-shrink-0 self-start rounded-[32px] bg-[#F4F6FB]">
                        <div className="flex w-full h-full flex-col items-start gap-[16px]">
                            ...
                            {hasActiveDeal === true ? (
                                <>
                                    <div className="flex flex-col gap-3 self-stretch p-4 rounded-2xl bg-zinc-200 border border-zinc-200">
                                    ...
                                    {resumeDealId && originalFlat && (user?.role === "manager" || user?.role === "admin") && (
                                        <Button
                                            onClick={() => {
                                                // Resume directly on sign step; paymentMethod is a required field for PayModal state.
                                                dispatch(openPay({
                                                    flat: originalFlat,
                                                    paymentMethod: "full",
                                                    step: "sign",
