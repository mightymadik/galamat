...
export async function POST(
  ...
  try {
    ...
    const { newCustomerDocumentId } = body as { newCustomerDocumentId?: string };
    ...
    await strapiAxios.put(
      ...
      {
        data: {
          customer: { connect: [newCustomerDocumentId] },
          dealStatus: "Договор подписан",
        },
      },
      { headers }
    );
    ...
    console.error("[deals/renewal/complete]", e?.response?.data ?? e);
